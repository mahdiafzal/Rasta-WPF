...
*/
class Xal_Extension_RayaDars_Lms
{
	...
	protected function	_getDataset($argus)
	{
		...
		$sql_params['fields'][2] = ' REPLACE(`path`,"__", CONCAT("$",`c_id`)) as `path`, `desc`, IF(`user_id`='.$login_user['id'].',"u","o") as `owner`, `is_public`, status FROM `unsorted_qu_an` ';
		...
		if($request['action']=="setpublic" | $request['action']=="unsetpublic" )
		{
			$is_public = ($request['action']=='setpublic')?1:0;
			$qn_status = ($is_public==1)?1:2;
			//return $entity;
			if(strpos($request['path'], '$'))
			{
				...
				if($last_entity_path = $this->DB->fetchOne($sql))
				{
					...
					//$result['debug'] = Rasta_Base34_Operation::addOneTo( '0z' );
					//return $result;
				}
				else
	...
	protected function GetQuestionDataset($entity, $sql_params, $result)
	{
		...
		$sql = 'SELECT '.$sql_params['fields'][2].' WHERE status!=0 AND level='.($entity['level']+1).' AND (`reg_time`> DATE_ADD(NOW(), INTERVAL -3 DAY) OR '.$sql_params['is-owner'].') AND ('.$sql_params['owner-or-admin'].') AND `path` LIKE "'.$entity['path'].'q__" '.$sql_params['order'] ;
	...
	protected function GetAnswerDataset($entity, $sql_params, $result)
	{
		...
		// n=Answer
		//$sql = 'SELECT '.$sql_params['fields'][3].' WHERE status=1 AND level='.($entity['level']+1).' AND '.$sql_params['member-of-user-group'].' AND `path` LIKE "'.$entity['path'].'q__" '.$sql_params['vote-query'].$sql_params['order-by-score'] ;
		$sql = 'SELECT '.$sql_params['fields'][3].' WHERE status=1 AND level='.($entity['level']+1).' AND '.$sql_params['member-of-user-group'].' AND `path` LIKE "'.$entity['path'].'n__" '.$sql_params['vote-query'].$sql_params['order-by-score'] ;
		//$sql = 'SELECT '.$sql_params['fields'][1].' WHERE status=1 AND level='.($entity['level']+1).' AND '.$sql_params['member-of-user-group'].' AND `path` LIKE "'.$entity['path'].'n__" '.$sql_params['vote-query'].$sql_params['order'] ;
		$result['Question'][$answer_view]['Answers'] = $this->DB->fetchAll($sql);

		// Unsorted Answers
		$sql = 'SELECT '.$sql_params['fields'][2].' WHERE status!=0 AND level='.($entity['level']+1).' AND `reg_time`> DATE_ADD(NOW(), INTERVAL -3 DAY) AND ('.$sql_params['member-of-admin-group'].') AND `path` LIKE "'.$entity['path'].'n__" '.$sql_params['order'] ;
...
// SELECT REPLACE(`path`,"__", CONCAT("$",`c_id`)) as `path`, `desc`, IF(`user_id`=1,"u","o") as `owner`, `is_public`, status FROM `unsorted_qu_an` WHERE status!=0 AND level=3 AND (`reg_time`> DATE_ADD(NOW(), INTERVAL -3 DAY) OR owner="u") AND ('.$sql_params['member-of-admin-group'].') AND `path` LIKE "1.q01n__"
