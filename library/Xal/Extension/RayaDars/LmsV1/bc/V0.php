	...
	protected function GetQuestionDataset($entity, $login_user, $sql_params, $result, $parent=null, $mode='get', $sets='all')
	{
		...
		if($mode=='get')
		{
			...
			if(($sets=='all' | $sets=='unsorted') & $selected_ask_editable=="y")
			{
				...
				$sql = 'SELECT '.$sql_params['table-fields-user-questions-with-score'].' WHERE `status` NOT IN (0,4) AND '.$sql_params['is-admin'].' AND (`path` LIKE "'. $questions_path_pattern.'" OR `prepath` LIKE "'. $entity['path'].'Q__") ORDER BY `id` DESC';
				...
				$sql = 'SELECT '.$sql_params['table-fields-user-questions-with-score'].' WHERE `status` NOT IN (0,4) AND '.$sql_params['is-admin'].' AND (`path` LIKE "'. $questions_path_pattern.'N__" OR `prepath` LIKE "'. $questions_path_pattern.'N__") ORDER BY `id` DESC';
