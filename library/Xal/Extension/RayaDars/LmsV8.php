...
class Xal_Extension_RayaDars_LmsV8
{
	...
	protected function	_getDataset($argus)
	{
		...
		// 	$sql = "SELECT *  FROM `user_quiz_start` WHERE `status`=1 AND (`quiz_result`='' OR `quiz_result` IS NULL) LIMIT 50;";
		// 	//$result['debug1'] = $sql;
	...
	protected function GetQuizTestsDataset($entity, $login_user, $sql_params, $result, $mode='start')
	{
		...
		// get quiz info and setting
		$sql = 'SELECT '.$sql_params['table-fields-courses-plan-started-ended-editable-prerequisite'].' WHERE ('.$sql_params['is-active-started-notended'].' OR '.$sql_params['is-admin'].') AND '.$sql_params['member-of-user-group'].' AND `path` LIKE "'.$entity['path'].'" ';
		if(!$quiz_result = $this->DB->fetchAll($sql)) return $result;
		$quiz_setting = json_decode($quiz_result[ 0 ]['desc'], true);
