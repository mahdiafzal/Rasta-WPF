...
*/
class Xal_Extension_RayaDars_LmsV8
{
	...
	protected function GetSurveyQuestionsDataset($request, $login_user, $sql_params, $result, $mode='start')
	{
		...
		$sql = 'SELECT '.$sql_params['table-fields-courses-started-ended-editable'].' WHERE ('.$sql_params['is-active-started-notended'].' OR '.$sql_params['is-admin'].') AND '.$sql_params['member-of-user-group'].' AND `path` LIKE "'.$request['path'].'" ';
	...
	protected function CheckPrerequisite($prerequisite)
	{
		$table = '';
		if(preg_match("/(^\d+\.s[\d\w]{2}$)|(^\d+\.l[\d\w]{2}s[\d\w]{2}$)/", $prerequisite['path']))
			$table = ' `user_survey_start` ';
		if(strlen($table)<2)	return true;
		$where = array();
		foreach ($prerequisite as $key => $value)
		{
			if(in_array($key, array('path', 'goto'))) continue;
			$where[] = " `$key`$value ";
		}
		$sql_where = '';
		if(count($where)>0) $sql_where = " AND (".implode(" AND ", $where).") ";
		$sql = "SELECT COUNT(*) FROM $table WHERE `path` LIKE '".$prerequisite['path']."' $sql_where";
