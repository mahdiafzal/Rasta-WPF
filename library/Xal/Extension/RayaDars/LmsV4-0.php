...
class Xal_Extension_RayaDars_LmsV4
{
	...
	protected function	_getDataset($argus)
	{
		...
		elseif ($request['namespace']=="Discussion" & $request['action']=="get")
		{
			// Discussion
			$result = $this->GetDiscussionDataset($entity, $login_user, $sql_params, $result);
		}
	...
	protected function GetDiscussionDataset($entity, $login_user, $sql_params, $result, $parent=null)
	{
		...
		// Discussions
		$sql = 'SELECT `d_id` as `id`, `status`, `reg_time` as `datatime`, `path`, `desc`, `lname`, `fname`, IF(`user_id`='.$login_user['id'].',"u","o") as `owner`, "'.$parent['editable'].'" as `editable`  FROM `user_discussion` WHERE `path` = "'.$entity['path'].'" ORDER BY `d_id` DESC '.$sql_params['limit'] ;
		//$result['debug2'] = $sql;
