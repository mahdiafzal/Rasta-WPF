...
*/
class Xal_Extension_RayaDars_LmsV5
{
	...
	protected function GetDiscussionDataset($request, $login_user, $sql_params, $result, $parent=null, $mode='get')
	{
		...
		if($mode=='get')
		{
			$sql = 'SELECT `d_id` as `id`, `status`, `reg_time` as `datetime`, `path`, `desc`, `lname`, `fname`, IF(`user_id`='.$login_user['id'].',"u","o") as `owner`, "'.$parent['editable'].'" as `editable`  FROM `user_discussion` WHERE (`status`=1 OR ('.$sql_params['is-owner'].' AND `status` IN (1,3,4)) OR ("'.$parent['editable'].'"="y")) AND `path` = "'.$request['path'].'" ORDER BY `d_id` DESC '.$sql_params['limit'] ;
			//$result['debug2'] = $sql;
			$result[$request_namespace]['Discussion'] = $this->DB->fetchAll($sql);
		}
		elseif ($mode=='push')
		{
			$sql = 'SELECT `d_id` as `id`, `status`, `reg_time` as `datetime`, `path`, `desc`, `lname`, `fname`, IF(`user_id`='.$login_user['id'].',"u","o") as `owner`, "'.$parent['editable'].'" as `editable`  FROM `user_discussion` WHERE `path` = "'.$request['path'].'" AND `d_id`>'.addslashes($request['action-params']['start']).' ORDER BY `d_id` DESC '.$sql_params['limit'] ;
			$result['prepend'][$request_namespace]['Discussion'] = $this->DB->fetchAll($sql);
			$sql = 'SELECT `d_id` as `id`, `status`, `reg_time` as `datetime`, `path`, `desc`, `lname`, `fname`, IF(`user_id`='.$login_user['id'].',"u","o") as `owner`, "'.$parent['editable'].'" as `editable`  FROM `user_discussion` WHERE `path` = "'.$request['path'].'" AND `d_id`<'.addslashes($request['action-params']['end']).' ORDER BY `d_id` DESC '.$sql_params['limit'] ;
	...
	protected function GetQuizDataset($entity, $sql_params, $result)
	{
		...
		$sql = 'SELECT '.$sql_params['table-fields-courses-started-ended'].' WHERE ('.$sql_params['is-active'].' OR '.$sql_params['is-admin'].') AND '.$sql_params['member-of-user-group'].' AND level='.($entity['level']+1).' AND `path` LIKE "'.$entity['path'].'z__" '.$sql_params['order'];
	...
	protected function GetQuizResults($request, $login_user, $sql_params, $result)
	{
		...
		$sql = "SELECT *  FROM `user_quiz_start` WHERE `user_id`=".$login_user['id']." AND `status`=1 AND `quiz_result`=''";
		...
		foreach ($userendedquizes as $key => $value)
		{
				...
				{
					$has_null_value = ture;
					$this->ComputeQuizResult($value, $login_user, true, true);
				}
	...
	protected function ComputeQuizResult($ended_quiz, $login_user, $need_to_update=false, $with_siblings=false)
	{
		...
		$sql = 'SELECT `t_id` as `id`, `correct`  FROM `quiz_tests` WHERE `t_id` IN ('.$ended_quiz['tests'].')'; //"AND `path` LIKE "'.$ended_quiz['path'].'"' ; //
	...
	protected function GetQuizTestsDataset($entity, $login_user, $sql_params, $result, $mode='start')
	{
		...
		// get quiz info and setting
		$sql = 'SELECT '.$sql_params['table-fields-courses-started-ended-editable'].' WHERE ('.$sql_params['is-active-started-notended'].' OR '.$sql_params['is-admin'].') AND '.$sql_params['member-of-user-group'].' AND `path` LIKE "'.$entity['path'].'" ';
		//$result['debug'] = $sql;
		if(!$quiz_result = $this->DB->fetchAll($sql)) return $result;
		$quiz_setting = json_decode($quiz_result[ 0 ]['desc'], true);
