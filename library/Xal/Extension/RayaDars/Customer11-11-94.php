...
class Xal_Extension_RayaDars_Customer
{
	...
	protected function GenNewCustomerCode()
	{
		$Num32Chars = "123456789BCDEFGHIJKLNOPQSTUVWXYZ";
		$seed = str_split($Num32Chars); // and any other characters
	...
	protected function	_new($argus)
	{
		...
		$sql = "SELECT * FROM `customer_info` WHERE cu_cellphone LIKE '".$argus["cellphone"]."' ;";
		
		if( $result = $this->DB->fetchAll($sql) ) return false;
		
		
		$codegenerationprogress = true;
		while($codegenerationprogress)
		{
			$cu_code = $this->GenNewCustomerCode();
			$sql = "SELECT * FROM `customer_info` WHERE cu_code LIKE '". $cu_code ."' ;";
		...
		$data["cu_code"]			= $cu_code; //$result[0]['cu_code'];
