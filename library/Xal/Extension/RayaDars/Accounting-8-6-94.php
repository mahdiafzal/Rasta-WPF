...
class Xal_Extension_RayaDars_Accounting
{
	...
	public function	run($argus)
	{
		...
		foreach($argus as $ark=>$argu)
		{
			switch($ark)
			{
				case 'reg.trans'	: return $this->_registerTransations($argu); break;
				case 'get.trans'	: return $this->_getTransaction($argu); break;
				case 'reg.bill2trans'	: return $this->_registerBill2Transaction($argu); break;
	...
	protected function	_registerBill2Transaction($argus)
	{
		$code = (is_numeric($_POST["transcode"]))?$_POST["transcode"]:0;
		$bill = (is_numeric($_POST["transbill"]))?$_POST["transbill"]:0;


		if($code==0 or $bill==0) return false;
		...
		$data['cellphone'] = (is_numeric($_POST["cellphone"]))?$_POST["cellphone"]:"";
		$data['address'] = (isset($_POST["address"]))?$_POST["address"]:"";
		
		$data['fname'] = addslashes( trim($data['fname']) );
		$data['lname'] = addslashes( trim($data['lname']) );
		$data['address'] = addslashes( trim($data['address']) );
		$data['cellphone'] = trim($data['cellphone']);
		
		if($code=="021")
		{
			$data['date'] = (isset($_POST["transdate"]))?$_POST["transdate"]:"1393-00-00";
			$data['code'] = $code . "00" . $data['bill_num'];
			$data['amount'] = (isset($_POST["transamount"]))?$_POST["transamount"]:"99";
			
			$this->DB->insert('transactions_bank', $data);
			return true;
		}
		if(!$this->DB->update('transactions_bank', $data, "`code`='".$code."'"))
			return false;
		return true;
	...
	protected function	_getTransaction($argus)
	{
		$code = (is_numeric($_POST["transcode"]))?$_POST["transcode"]:0;
		if($code==0) return false;
		if( !is_object($this->DB) )	$this->DB = Zend_Registry::get('extra_db_rd_data');
		
		$sql = "SELECT * FROM `transactions_bank` WHERE `code` = '".$code."'; ";
		if(! $result = $this->DB->fetchAll($sql)) return array();
		return $result[0];
		
	}
	
	protected function	_registerTransations($argus)
	{
		if( !is_object($this->DB) )	$this->DB = Zend_Registry::get('extra_db_rd_data');
		
		$errors = array();
		
		$data["bank_id"]		= (isset($_POST["transbank"]))?$_POST["transbank"]:1;
		$data["date"]			= (isset($_POST["transdate"]))?($_POST["transdate"]["year"].':'.$_POST["transdate"]["month"].':'.$_POST["transdate"]["day"]):'';
		
		if(is_array($_POST["trans"]))
			foreach($_POST["trans"]['code'] as $k=>$v)
			{
				$data['code'] = trim( $_POST["trans"]['code'][$k] );
				$data['amount'] = trim( $_POST["trans"]['amount'][$k] );
				if( empty( $data['code'] ) or empty( $data['amount'] ) ) continue;
				//$data["code"]	= $trans['code'];
				//$data["amount"]	= $trans['amount'];
				//print_r($data);
				try
				{
					if(!$this->DB->insert('transactions_bank', $data)) $errors[] = $trans['code'];	
				}
	...
	protected function	_regOrder($argus)
	{
		...
		{
			if(!is_array($argus['pishkhan'])) return array('status'=>false, 'message'=>'خطا در ثبت سفارش؛ دفتر پیشخوان دوباره وارد سیستم شوید.');;
		...
		if($this->DB->insert('customer_order', $data))
		{
			...
			return array('status'=>true, 'attach'=>array('orderID'=>$data['co_id']));
		}
		return $_return;
	}
