<?php
	class Xal_Extension_Ershad_RegisterationPr
	{
		...
		{
			if(isset($_SESSION['KanoonRegApp']))
			{
				if(is_array($_SESSION['KanoonRegApp']['LoginUser']))
				{
					return $_SESSION['KanoonRegApp']['LoginUser']['Email'];
				}
			}
			return 0;
		}
		protected function _logoutUser($argus)
		{
			if(isset($_SESSION['KanoonRegApp']))
			{
				$_SESSION['KanoonRegApp']['LoginUser'] = '';
			}
			return true;
		}
		protected function _loginUser($argus)
		{
			...
			$users = $this->MDB->users->find($document);
			if($users->count()>1) return -3;
			$User = $users->getNext();
			
			if(isset($User['KanoonId']))
			{
				$query['_id']= $User['KanoonId'];
				$fields = array('Metadata', 'Progress');
				return $this->MDB->kanoons->findOne($query, $fields);
			}
			return 1;
		...
		{
			if( !is_object($this->MDB) or  $this->MDB==NULL)	$this->MDB = $this->DbConnect();
			//$colections = array('kanoons', 'users', 'persons');
			//$kanoon = $user = $person = '';
			if(isset($_SESSION['KanoonRegApp']))
			{
				if(is_array($_SESSION['KanoonRegApp']['LoginUser']))
			...
			if(is_array($_POST['kanoon']))
				if(is_array($_POST['kanoon']['LoginInfos']))
				{
					...
					//return $this->GetKanoonUser($user);
		...
		{
			if(!isset($_SESSION['KanoonRegApp']))	$_SESSION['KanoonRegApp'] = array();
			$_SESSION['KanoonRegApp']['LoginUser'] = $user;
			//$ses = new Zend_Session_Namespace('Zend_Auth');
			
			//$ses->storage->id = $user['_id'];
			
			//$data['id'] = $user['_id'];
			//$data['all_groups'] = "/3/";
			//$data['is_admin'] = 0;
			//$ses->getStorage()->write($data);
		}
		...
		protected function	MultiFileUploader($destinations, $valids)
		{
			...
			return array('destinations'=>$destinations, 'erros'=>$error);
		}
