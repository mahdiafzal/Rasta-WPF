<?php
	class Xal_Extension_Ershad_RegisterationPr
	{
		...
		protected function _collectionMap($variable) {
			$collections = array(
				'kanoon'	=> 'kanoons',
				'kanoons'	=> 'kanoons',
				'person'	=> 'person',
			...
			// TODO: check if null
			return $collections[$variable];
		}
		
		/*
		 * _getDataTest
		 * call this to get data of current user form db 
		 * this is just test ;)
		 * use this for development view &appenv=development
		 */
		protected function _getDataTest($argus)
		{

			/* check user authorized */
			if(!$user = $this->IsUserLogin()) return -1;

			/* check user role and set $kanoonID */
			$KanoonId = ($user['role']=='kanoon') ? $user['KanoonId'] : null;

			/* check and get schemas */
			...
			try{
				...
				var_dump($user);
				var_dump($schemas);
				var_dump($this->MDB->getCollectionNames());
				foreach ($schemas as $shema) {

					/* always first part defines collection name  */
					$explodedShema = explode('.', $shema);
					$collectionDefiner = $explodedShema[0];

					/* get collection name */
					$collection = $this->_collectionMap($collectionDefiner);
					$KanoonId = new MongoId($KanoonId);

					// $CollectionArray = iterator_to_array($this->MDB->$collection->find(array('_id' => $KanoonId )));
					$CollectionArray =	iterator_to_array($this->MDB->$collection->find(
											array('_id' => array('$eq' => $KanoonId)),
											array('Progress' => 0)
										));
		...
		protected function _updateInfos($argus)
		{
		...
		{ 
			if(empty($_POST['_id'])) return -1;
			$KanoonId = Rasta_Util_DotNotation::GetValue($_SESSION, "KanoonRegApp.LoginUser.KanoonId");
			$_id = $_POST['_id'];
			$_collection = $_POST['dataset'][0];
			$fields = $query = array(); //$pull for embed doc 
			$embededdoc = true;
			switch($_collection)
			{
				case "person":
			...
			try
			{
				$query['_id'] = new MongoId($_id);
				$query['Metadata.KanoonId']= new MongoId($KanoonId);
				if(!$embededdoc)
					$this->MDB->$collection->remove($query);
				else
					$this->MDB->$collection->update($query, $fields);
				return 1;
			...
			foreach($schemas as $schema)
			{
				...
				if( preg_match($pattern, $schema, $matches))
					if(isset($matches[4]))
					{
						...
						{
							//if(!in_array($user['role'], array('staff'))) return -5;
							$KanoonId = (isset($_REQUEST['_id']) & strlen($_REQUEST['_id'])>3 )?$_REQUEST['_id']:null;
				...
				if( preg_match($pattern, $schema, $matches))
					...
					{
						if($KanoonId==null)
							if(!$this->IsPermitted($user)) return -5;
						$PersonId = (isset($_REQUEST['_id']) & strlen($_REQUEST['_id'])>3 )?$_REQUEST['_id']:null;
				...
				if(isset($matches[3]))
				{
					...
					$refrences['KanoonId'] = ($matches[2]=='kanoon')?(($KanoonId!=null)?$KanoonId:$_REQUEST['_id']):null;
					$refrences['PersonId'] = ($matches[2]=='person')?$_REQUEST['_id']:null;
					if(isset($query['refrence']))
						foreach($query['refrence'] as $key=>$value)
							if(isset($refrences[$value]))
								$query['statement'][$key] = new MongoId($refrences[$value]);
		...
		protected function IsPermitted($user, $roles=array('staff'))
		{
			if(!is_array($roles)) return false;
			if(in_array('ALL',$roles)) return true;
			if(!in_array($user['role'], $roles)) return false;
		}
		...
		protected function RegisterPerson()
		{
			...
			if(is_array($document['_id']))
			{
				if(strlen($document['_id']['$id'])==24)	$document['_id']= new MongoId($document['_id']['$id']);
				else unset($document['_id']);
			}
			elseif(isset($document['_id'])) unset($document['_id']);
			//if(!empty($kanoon))
