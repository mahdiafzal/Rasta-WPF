...
/// string functions
	...
	protected function sys_if($fn_tree, $pre_vars)
	{
		...
		$_return = false;
		if(isset($fn_tree['is']))
			$_return = !!($this->_axmlExecution($fn_tree['is'], $pre_vars));
		elseif(isset($fn_tree['isnot']))
			$_return = !($this->_axmlExecution($fn_tree['isnot'], $pre_vars));
		if($_return) $pre_vars = $this->sys_execution($fn_tree['execution'], $pre_vars);
		elseif(isset($fn_tree['else']['execution'])) $pre_vars = $this->sys_execution($fn_tree['else']['execution'], $pre_vars);
		return $pre_vars;
