$_(function() {
	...
	$_('#pagethemelist').find('.themeIcon').each(function(){
						themeid			= $_(this).index()+1
						$_(this).css('background-image','url(/flsimgs/public/RSDthemepic/'+themeHref[4]+'-'+themeid+'.gif)')
														 });
	if(themeHref[4]==9)		$_('#pagethemelist').find('.themeIcon').nextAll('a:last').remove();
	if(themeHref[4]==11)	$_('#pagethemelist').find('.themeIcon').not('a:eq(0)').remove();
	$_('#mlmthemelist').find('.themeIcon').button();
	$_('#mlmthemelist').find('.themeOption').button();
		...
		buttons: [
					{
						...
						{ 
							...
							Ctoolbar.fn.saveSuccess=function(msg){ if(msg[0]==true) window.location.reload(); else $var.alert(msg[1]); } ;
						...
						.click(function()
							{
								...
								{
									tempSelectWin = null;
									tempSelectWin	=	window.open('/admin/index/template','tempSelectWin',
