...
function getPageProperties()
{
	$_.ajax({
			...
			type		: 'post',
			dataType	: 'json',
			data		: {'pageID': $var.config.site.pageId},
...
function rastaAlert(msg, inButton, inWidth)
{
	if(typeof inWidth == 'undefined') inWidth = 300;
	if(typeof inButton=='undefined') inButton =null;
	var _buttons = [
					{
						text: "بستن",
						click: function() { $_(this).dialog("destroy"); }
					}
				];
	if(inButton!=null)
	{
		_buttons = [
					inButton,
					{
						text: "بستن",
						click: function() { 
							$_(this).dialog("destroy");
						  }
					}
				];
	}
	$_('body>#rastaAlert').remove();
	$_('<div id="rastaAlert"></div>')
		.html(msg)
		.dialog({ buttons: _buttons , width: inWidth , close: function( event, ui ) {$_(this).dialog("destroy");} });
}
