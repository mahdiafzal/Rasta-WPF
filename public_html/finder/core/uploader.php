...
class uploader {
    ...
    public function __construct() {
        ...
        if (!is_dir($this->config['uploadDir']))
		{
			...
			chmod($this->config['uploadDir'], $this->config['dirPerms']);
		}
        ...
        if (!is_dir($this->typeDir)) {
			...
			chmod($this->typeDir, $this->config['dirPerms']);
        ...
        if ($config['disabled'] || !$config['access']['files']['upload']) {
            ...
            if (!strlen($message)) {
                if (!is_dir(path::normalize($dir)))
				{
					...
					chmod(path::normalize($dir), $this->config['dirPerms']);
				}
    ...
    protected function makeThumb($file, $overwrite=true) {
        ...
        chmod($thumbDir, $this->config['dirPerms']);
