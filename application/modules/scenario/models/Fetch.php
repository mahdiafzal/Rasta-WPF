...
class Scenario_Model_Fetch
{
	...
	public function	getScenrioConfiguration()
	{
		...
		/// Fields
		$config['fields']	= 'co.id , co.title AS `title1`, co.ltn_name AS `title2`, co.publish_up AS `datetime`, co.description AS `abstract`, co.content AS `text` ';
		if(!empty($this->contentPros['data.fields']))
		{
			...
			$fl_ad	= array('id'=>'co.id ', 'title1'=>'co.title AS `title1` ', 'title2'=>'co.ltn_name AS `title2` ', 'abstract'=>'co.description AS `abstract` ', 'text'=>'co.content AS `text` ', 'datetime'=>'co.publish_up AS `datetime` ' );
	...
	public function	getScenrioRtcs()
	{
		...
		$sql_0	= 'SELECT '.$this->configs['fields'].', co.type_id, me.keywords, me.description, me.author, me.extra_data ';
		...
		{
			$relation	= " OR ";
			if(isset($this->configs['find']['relation']))
			{
				$relation	= $this->configs['find']['relation'];
				if($relation=="OR" or $relation=="AND" or $relation=="or" or $relation=="and")
					$relation = " ".$relation." ";
				else
					$relation	= " OR ";
			}
			...
			{
				$sql_4_parts[]	= ' (co.`title` LIKE "'.$this->configs['find']['title1'].'") ';
			}
			if(isset($this->configs['find']['title2']))
			{
				$sql_4_parts[]	= ' (co.`ltn_name` LIKE "'.$this->configs['find']['title2'].'") ';
			}
			if(isset($this->configs['find']['abstract']))
			{
				$sql_4_parts[]	= ' (co.`description` LIKE "'.$this->configs['find']['abstract'].'") ';
			}
			if(isset($this->configs['find']['text']))
			{
				$sql_4_parts[]	= ' (co.`content` LIKE "'.$this->configs['find']['text'].'") ';
			}

			if(count($sql_4_parts)>0) $sql_4	.= " AND (" .implode($relation, $sql_4_parts). " )";
		}
	...
	public function fetchAll($mode, $page_xml=array())
	{
		...
		{
			if($this->configs['block']=='metablock')	return array($this->_scenMetaBlocker($rtcData), $page_xml);
			else										return '';
		}
		...
		{
			case 'html':
				if(isset($this->contentPros['xal']) and strlen($this->contentPros['xal'])>4)
				{
					foreach($rtcData['content'] as $key=>$rtc)
					{
						if( !is_object($this->_XAL) ) $this->helper_ignite_XAL();
						$this->_XAL->setTheRunningMode('NORMAL_MODE');
						$page_xml['var:rtc']=$rtc;
						$page_xml = $this->_XAL->run('<execution>'.$this->contentPros['xal'].'</execution>', $page_xml);
						$rtcData['content'][$key] = $page_xml['var:rtc'];
						unset($page_xml['var:rtc']);
					}
				}
				if($this->configs['block']=='normal')			return array($this->_scenBlocker($rtcData), $page_xml);
				elseif($this->configs['block']=='metablock')	return array($this->_scenMetaBlocker($rtcData), $page_xml);
				break;
			case 'json':
	...
	protected function	_scenMetaBlocker($rtcs)
	{
		...
		if( !$rtcs )
		{
			...
			$sysParams['#rasta-type#']					= 'scenario';
			$sysParams['#rasta-scenario-fields#']		= $this->contentPros['data.fields'];
			$sysParams['#rasta-scenario-count#']		= $this->configs['count'];
		...
		if($this->htmlBlock['bm_type']==1)
		{
			...
			$js_out	= '<script type="text/javascript" language="javascript"> rasta_scenario={'; //['.$this->scenario['id'].']';
			...
			$sysParams['#rasta-unic#']					= $this->scenario['id'];
			$sysParams['#rasta-type#']					= 'scenario';
			$sysParams['#rasta-scenario-fields#']		= ($this->renderer=='admin')?$this->contentPros['data.fields']:"";
			$sysParams['#rasta-scenario-count#']		= ($this->renderer=='admin')?$this->configs['count']:"";
			...
			$sysParams['#rasta-blockheader2#']			= $this->scenario['latin_title'];
			$sysParams['#rasta-content-datetime#']		= '';
			$sysParams['#rasta-scenario-js#']= $js_out;

			return str_replace(array_keys($sysParams), array_values($sysParams), $this->htmlBlock['block']);
		}
		elseif($this->htmlBlock['bm_type']==2)
		{
			if( !is_object($this->_XAL) )	$this->helper_ignite_XAL();
			$this->_XAL->setTheRunningMode('SAFE_MODE');
			$ext_data	= '<execution>'.stripslashes($this->htmlBlock['bm_code']).'</execution>';
			$metablocks	= $this->_XAL->run($ext_data, array() );
			if(!is_array($metablocks['var:metablocks'])) return '';

			$block_sysParams = array();
			$block_sysParams['#rasta-blockcontent#']					= '';
			$block_sysParams['#rasta-blockcontent-abstract#']	= '';

			foreach ($metablocks['var:metablocks'] as $key => $value)
			{
				$metaout = array();
				foreach($rtcs['content'] as $con)
				{
					...
					if( strlen( trim($con['extra_data']) )>4 )	$con['user_params']	= $this->_parseUserParams($con['extra_data']);
					$metablock_sysParams = array();
					$metablock_sysParams['#rasta-content-datetime#']		= (isset($con['datetime']))?Application_Model_Localize::datetime($con['datetime']):'';
					$metablock_sysParams['#rasta-blockcontent#']			= (isset($con['text']))?$con['text']:'';
					$metablock_sysParams['#rasta-blockheader#']			= ($this->page['page_dir']==2 )?$con['title2']:$con['title1'];
					$metablock_sysParams['#rasta-blockheader2#']			= $con['title2'];
					$metablock_sysParams['#rasta-unic#']					= (isset($con['id']))?$con['id']:'';
					$metablock_sysParams['#rasta-blockcontent-abstract#']	= (isset($con['abstract']))?$con['abstract']:'';

					$tmpout	= str_replace(array_keys($metablock_sysParams), array_values($metablock_sysParams), $value['repeat']);
					if(is_array($con['user_params']))
						$tmpout	= str_replace(array_keys($con['user_params']), array_values($con['user_params']), $tmpout);
					$metaout[]	= $tmpout;
				}
				$block_sysParams['#rasta-'.$key.'#'] = $value['prefix'].' '.implode('', $metaout).' '.$value['postfix'];
				//return "FFFFFFFFDDGGHHJJKK".$block_sysParams['#rasta-'.$key.'#'];
			}


			$block_sysParams['#rasta-unic#']			= $this->scenario['id'];
			$block_sysParams['#rasta-type#']			= 'scenario';
			$block_sysParams['#rasta-scenario-fields#']	= ($this->renderer=='admin')?$this->contentPros['data.fields']:"";
			$block_sysParams['#rasta-scenario-count#']	= ($this->renderer=='admin')?$this->configs['count']:"";
			$block_sysParams['#rasta-blockheader#']		= $this->scenario['title'];
			$block_sysParams['#rasta-blockheader2#']	= $this->scenario['latin_title'];
			$block_sysParams['#rasta-content-datetime#']= '';

			$js_out	= '<script type="text/javascript" language="javascript"> rasta_scenario={';
			if($this->configs['paging']=='true')	$js_out	= $js_out.'count:'.$rtcs['count'];
			$js_out	= $js_out.'}</script>';
			$block_sysParams['#rasta-scenario-js#']= $js_out;
			return str_replace(array_keys($block_sysParams), array_values($block_sysParams), $this->htmlBlock['block']);
