...
class Scenario_Model_Fetch //extends Application_Model_Page_Free
{
	...
	{
		$config['count']	= (isset($this->scenario['xal']['var:config']['count']))?$this->scenario['xal']['var:config']['count']:10;
		...
		//print_r(array_intersect( array_map( trim, explode(',', 'title1, hjk, k') ) , array('id','title1','title2','abstract','text', 'datetime') )); die();
	...
	public function	getScenrioRtcs()
	{
		...
		$sql_3	= ' WHERE '.Application_Model_Pubcon::get(1111, 'co')
				. ' AND co.type_id NOT IN (SELECT ts_ct_id FROM `wbs_content_type_setting` WHERE ts_status=0 OR ts_data_sc="/0/")'
