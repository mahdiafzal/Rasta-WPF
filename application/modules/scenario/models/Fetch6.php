...
class Scenario_Model_Fetch //extends Application_Model_Page_Free
{
	...
	{
		$config['count']	= (isset($this->scenario['xal']['var:config']['count']))?$this->scenario['xal']['var:config']['count']:30;
		$config['count']	= (is_numeric($this->contentPros['data.count']))?$this->contentPros['data.count']:$config['count'];
		$config['start']	= (isset($this->scenario['xal']['var:config']['start']))?$this->scenario['xal']['var:config']['start']:0;
	...
	public function	getScenrioRtcs()
	{
		...
		if(is_array($this->configs['find']))
		{
			...
			{
				$sql_4	.= ' AND (co.`title` LIKE "'.$this->configs['taxoterm'].'") ';
			}
		}
