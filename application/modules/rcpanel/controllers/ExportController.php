<?php
class Rcpanel_ExportController extends Zend_Controller_Action
{
	var $DB;
	var $ses;
	var $prifix;
    ...
    public function frmwordpressAction()
    {
		...
		$this->view->assign('user_id',USRiD); 
	...
	public function get_site_data()
	{
		...
		$this->_helper->flashMessenger->addMessage(array('شما پس از ایجاد سایت می توانید از داده های خود پشتیبان تهیه کنید'));
   	...
   	public function wordpressAction()
    {
    	...
    	if($this->getRequest()->getParam('db_prefix')!='')	$this->prifix	= $this->getRequest()->getParam('db_prefix');
		else												$this->prifix	= 'wp_';
		...
		$fname			= "wb_".$this->Site_data[wb_id]."_".time().".sql";
		...
		$zipFname	= "wb_export_".$this->Site_data[wb_id]."_".time().".zip";
    ...
    public function expscenario()
    {
    	...
    	$sql	= 'select `id`,`title`,`latin_title` from `wbs_scenario` where wbs_id=' . $this->Site_data[wb_id] ;	
    ...
    {
		//fetch data from rastak
    	$sql	= 'select `ltn_name`,`title`,`description`,`crt_date`,`is_published`,`content`,`scenarios` from `wbs_rtcs` where `wbs_id`=' . $this->Site_data[wb_id] ;	
    	$result	= $this->DB->fetchAll($sql);
    	//---
    	$prifix	= $this->prifix;
    	$sql='';
    ...
    {
    	$sql	= 'select `title`,`url` from `wbs_links` where wbs_id=' . $this->Site_data[wb_id] ;	
    	$result	= $this->DB->fetchAll($sql);
    	//---
    	$prifix	= $this->prifix;
    	$tbl_name= 'links';
    ...
    public function expsitedata()
    {
    	...
    	$prifix	= $this->prifix;
    	$tbl_name= 'options';		
    	$sql	 = sprintf("UPDATE `%s%s` SET `option_value` = '%s' WHERE `wp_options`.`option_name` ='blogname' ;",$prifix,$tbl_name,addslashes($this->Site_data['wb_title']));    	
    	$sql	 .= sprintf("UPDATE `%s%s` SET `option_value` = '%s' WHERE `wp_options`.`option_name` ='blogdescription' ;",$prifix,$tbl_name,addslashes($this->Site_data['wb_description']));    	
    	return $sql;
   	}
