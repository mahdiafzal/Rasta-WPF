...
class Godpanel_PanelController extends Zend_Controller_Action
{
	...
	public function frmlistsiteAction()
    {
		...
		$sql2	= "select * from `wbs_domain` where `wb_id` in (".$wbID.") GROUP BY `wb_id`";
...
//				}
	...
	public function frmeditsiteAction()
    {
		...
		if (is_numeric($this->params['id']))	$wb_id	= $this->params['id'];
		else
		{
			$this->_helper->flashMessenger->addMessage('خطا در آرگومانهای ورودی');
			$this->_redirect('/godpanel/panel/frmlistsite');
		}
		$sql	='SELECT * FROM `wbs_profile` WHERE `wb_id`='.addslashes($wb_id);
...
//		}	    
	...
	{
		if(!is_array($this->params['wbgroup']) || count($this->params['wbgroup'])==0) return '/1/';
		sort($this->params['wbgroup']);
		$ugroup		= '/1/'. implode('/', $this->params['wbgroup']).'/';
		return $ugroup;
	}
	...
	public function editsiteAction()
    {
		$this->_helper->viewRenderer->setNoRender();
		$this->params	= $this->_getAllParams();
		if(trim($this->params['s_title'])=='') 		 
		{
			$this->_helper->flashMessenger->addMessage('لطفا عنوان پورتال را وارد کنید');
			if (is_numeric($this->params['id']))		$this->_redirect('/godpanel/panel/frmeditsite/id/'.$this->params['id']);
		...
		else 
		{
			try
			{
				...
				//add other values to $data
				$data ['wb_title'	]	= $this->params['s_title'];
				$data ['latin_title']	= $this->params['s_latintitle'];
				$data ['wb_description']= $this->params['s_description'];
				$data ['wb_slogan'	]	= $this->params['s_slogan'];
				$data ['wb_authors'	]	= $this->params['s_authors'];
				$data ['wb_keywords']	= $this->params['s_keywords'];
				$data['wbs_group'] 		= $this->_genWbGroupString();

				$this->DB->update('wbs_profile',$data ,'`wb_id`='.addslashes($this->params['id']) );
...
//		}
    ...
    {
		$wbs_id	= $this->_getParam('id');
		if(!is_numeric($wbs_id))	$this->_redirect($_SERVER['HTTP_REFERER']);
		$sql	= 'SELECT dm.*, pr.wb_title FROM wbs_domain AS dm, wbs_profile AS pr WHERE dm.wb_id=pr.wb_id AND  dm.wb_id ='.addslashes($wbs_id);
    ...
    public function unparkdomainAction()
    {
		...
		$sql	= 'SELECT * FROM wbs_domain WHERE (wbs_domain.domain not like "%'.$domainsuffix.'") and '
				  .'(wbs_domain.domain="'.$domain.'") and (wb_id = '.$wbs_id.')';
    ...
    public function parkdomainAction()
    {
		...
		if ($validator->checkdomain($domain)==1)
		{
			try 
			{
				$sql	= 'select `wb_id` from `wbs_profile` where `wb_id`='.$wbs_id;
				if(!$result= $this->DB->fetchAll($sql))
				{
					$this->_helper->flashMessenger->addMessage('پورتال انتخاب شده صحیح نمی باشد.');
					$this->_redirect('/godpanel/panel/');
				}
...
//		}
		...
		//ckeditor	
		$this->view->headScript	()->appendFile('/lib/ckeditor/ckeditor.js');
		//-------
	}
...
//		}
		...
		//$body	=  '<body dir="rtl"><div style=" width: 600px; border: 0px solid black; margin: 0pt auto; text-align: right; -moz-border-radius:8px; padding: 10px;">';
		$body	=  '<body dir="rtl"><div style="text-align: right;">';
		$body 	.= $text;
		$body	.=  '</div></body>';
		try 
		{
