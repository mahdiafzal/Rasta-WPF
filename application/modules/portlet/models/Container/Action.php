...
class Portlet_Model_Container_Action
{
	...
	{
		$this->helper_run_scripts($this->_layout);
...
//		$result	= $this->_XAL->run($action);
	...
	{
		if(isset($this->_preVars['var:Router']))
			unset($this->_preVars['var:Router']);
		...
		{
			$params = array();
			if(isset($this->_preVars['var:Router']['controller']))
				if($this->_params['controller']!= $this->_preVars['var:Router']['controller'])
					$params['controller'] = $this->_preVars['var:Router']['controller'];
			
			if(isset($this->_preVars['var:Router']['action']))
				$params['action'] = $this->_preVars['var:Router']['action'];			
			$this->setParams($params);
			if(! $this->_fetchScripts() ) die(Application_Model_Messages::message(404));
	...
	protected function	_getLayout()
	{
		...
		if(!isset($this->_preVars['var:DisableActionLayout']))
			if( $skin1 = $this->_fetchLayout($this->_action['ac_layout']) )	$skin.=$skin1;
		if(!isset($this->_preVars['var:DisableControllerLayout']))
			if( $skin2 = $this->_fetchLayout($this->_controller['cr_layout']) )	$skin.=$skin2;
		if(!isset($this->_preVars['var:DisablePortletLayout']))
			if( $skin3 = $this->_fetchLayout($this->_module['pr_layout']) )	$skin.=$skin3;
		return $skin;
	}
	protected function	_fetchLayout($lid)
	{
		...
		if(!$result	= $this->_DB->fetchAll($sql)) return '';
		return $result[0]['ly_code'];
	}
	...
	protected function	helper_run_scripts($code)
	{
		...
		$this->_preVars = $this->_XAL->run($code, $this->_preVars);
		return $this->_preVars;
	}
