...
class Usermanager_FrmlistController extends Zend_Controller_Action
{
    ...
    public function indexAction()
    {
		...
		$limit	= 25;
		$sql	= 'select * from `users` where `wb_user_id` = '. WBSiD .' ORDER BY `id` DESC limit '.$start.','.$limit;
		$result	= $this->DB->fetchAll($sql);
		$count	= $this->DB->fetchAll('select count(id) as `cnt` from `users` where `wb_user_id` = '. WBSiD);
