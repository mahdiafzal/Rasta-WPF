...
class Application_Model_Pubcon
{
	...
	static function get($mode=1111, $ns1='', $ns2='') 
	{
		...
		if(!empty($_SESSION['MyApp']['extra_user_groups']))
		{
			$part1 = $part2 = array();
			if(strlen($usRg)>0) $part1 = explode("/",$usRg);
			if(strlen($_SESSION['MyApp']['extra_user_groups'])>0) $part2 = explode("/",$_SESSION['MyApp']['extra_user_groups']);
			
			$usRg = "/".implode("/", array_unique( array_filter( array_merge($part1,$part2))))."/";
		}
