...
class Application_Model_Scenario_Commander
{
	...
	{
		$sql		= "SELECT *  FROM `wbs_scenario_allsubs` WHERE `wbs_id`='".WBSiD."' AND `sc_id`=".$this->scenario['id'];
...
//				. " AND `scenarios` LIKE '%/".$this->scenario['id']."/%' ORDER BY `wbs_rtcs`.`publish_up` DESC LIMIT ".$sql_start." , ".$this->properties['count'];
		$sql	= "SELECT `id` FROM `wbs_rtcs` WHERE wbs_id='".WBSiD. "' AND `is_published` != '0' AND `publish_up`<=NOW()  AND `publish_down`>=NOW()"
				. " AND (`scenarios` LIKE '%/".$this->getScenrioFamily()."/%') ORDER BY `wbs_rtcs`.`publish_up` DESC LIMIT ".$sql_start." , ".$this->properties['count'];
		$result	= $this->DB->fetchAll($sql);
