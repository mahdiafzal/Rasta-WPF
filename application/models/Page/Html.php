...
class Application_Model_Page_Html extends Application_Model_Page_Content
{
	...
	public function	getHtmlHead()
	{
		...
		$HtmlHead =	$this->injectSysParamsValue($HtmlHead);
		return $this->injectCustomVariables($HtmlHead);
	}
	...
	public function injectSysParamsValue($input)
	{
		...
		$sysParams['#rasta-host-root#']			= $this->docroot.'/'.WBSiD;
		$sysParams['#rasta-thumbs-root#']		= $this->docroot.'/'.WBSiD.'/.thumbs';
		$sysParams['#rasta-skinroot#']			= ($this->skin['wbs_id']>0)?$this->docroot.'/'.WBSiD.'/files'.$this->skin['skin_path']:$this->skin['skin_path'];
	...
	public function injectCustomVariables($input)
	{
		...
		if(is_array($this->page['wb_xml']['var:CustomVariables']))
		{
			$result = array();
			foreach($this->page['wb_xml']['var:CustomVariables'] as $key=>$value)
			{
		...
		if(is_array($variables) & !empty($variables[5]))
		{
			if( !is_object($this->_XAL) ) $this->helper_ignite_XAL();
			$this->_XAL->setTheRunningMode('NORMAL_MODE');
		...
		if(is_array($variables) & !empty($variables[1]))
		{
			if( !is_object($this->_XAL) ) $this->helper_ignite_XAL();
			$this->_XAL->setTheRunningMode('NORMAL_MODE');
