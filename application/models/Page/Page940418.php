...
class Application_Model_Page_Page
{
	...
	public function	__construct($data)
	{
		...
		$this->SkinVersin = ( isset($this->skin['skin_version']) )?$this->skin['skin_version']:1;
	...
	public function	getPage()
	{
		...
		$this->_XAL->enable(array('execution', 'tree', 'item', 'if', 'param', 'param.get', 'param.post', 'param.env', 'var'));
...
//		$sql	=	 "SELECT sk.`skin_id`, sk.`wbs_id`, `sk`.`skin_path`,`sk`.`skin_blocks`, `bd`.`body_id`, `bd`.`body`"
		$sql	=	 "SELECT sk.*, `bd`.`body_id`, `bd`.`body`"
