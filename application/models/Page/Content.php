...
class Application_Model_Page_Content extends Application_Model_Page_Page
{
	...
	/***
		1. analyzePageContentDeclaration() ---> add data to $this->allPageContents & $this->allPageSegments
		2. getPageSegments()
	***/
	...
	public function analyzePageContentDeclaration()
	{
		...
		//$namespace = (isset($_REQUEST['page-namespace']))?trim($_REQUEST['page-namespace']):'';
		$namespace = (isset($this->page['wb_xml']['var:ns']))?$this->page['wb_xml']['var:ns']:'';
		...
		foreach($this->page['wb_xml']['var:contents'] as $content)
		{
			...
			{
				if(isset($content['ns']))
					if(strlen( $content['ns'] )>0)
						if($content['ns']!=$namespace)
							continue;
			}
	...
	public function	getPageRTC()
	{
		...
		$sql	= 'SELECT co.id AS unic, co.title AS title1, co.ltn_name AS title2, co.description AS abstract, co.publish_up , co.content AS text, co.setting, co.type_id,'
				. ' me.keywords, me.description, me.author,  me.extra_data '
	...
	public function getPageScenarios()
	{
		...
		foreach($result as $raw_scenario)
		{
			...
			if(isset($this->allPageContents['s']['data'][$raw_scenario['id']]['skin.block.section']))
			{
				if(isset($this->htmlBlocks[ $this->allPageContents['s']['data'][$raw_scenario['id']]['skin.block.section'] ]['c']))
					$data['html.block']	= $this->htmlBlocks[ $this->allPageContents['s']['data'][$raw_scenario['id']]['skin.block.section'] ]['c'];
			}
			if(empty($data['html.block']))
				$data['html.block']	= $this->htmlBlocks[ $this->allPageContents['s']['data'][$raw_scenario['id']]['container'] ]['c'];							
			
			//$data['html.block.extra'] 	= (isset($this->htmlBlocks[0]['c']))?:;
			$data['renderer'] 			= $this->renderer;
			$obj	= new Scenario_Model_Fetch($data);
			$result = $obj->fetchAll('html', $this->page['wb_xml']);
			$scenario[$raw_scenario['id']] = $result[0];
			if(is_array($result[1]) & count($result[1])>0)
				$this->page['wb_xml'] = $result[1];
			//print_r($this->page['wb_xml']); die();
		}
		return $scenario;
	}
	...
	public function getPageSegments()
	{
		...
		foreach($this->allPageSegments as $section_id=>$s_contents)
		{
			...
			foreach($s_contents as $content)
			{
				...
				$skin_block_section = (isset($content['skin.block.section']))?$content['skin.block.section']:$section_id;
				
				if($type=='t' && !empty($this->pageRTC[$content_id]))
					$segment[$section_id][$i]	=	$this->getBlockedContent($skin_block_section, 'rtc', $this->pageRTC[$content_id]);
				elseif($type=='q' && !empty($this->SmMenus[$content_id]))
					$segment[$section_id][$i]	=	$this->getBlockedContent($skin_block_section, 'menu', $this->SmMenus[$content_id]);
				elseif($type=='g' && !empty($this->gallery[$content_id]))
					$segment[$section_id][$i]	=	$this->getBlockedContent($skin_block_section, 'gallery', $this->gallery[$content_id]);
	...
	public function getBlockedContent($section, $type, $content)
	{
		...
		if($stype == 'n')
		{
			...
			if($type == 'rtc')
			{
				...
				$sysParams['#rasta-content-date#']			= (!empty($content['date']))?$content['date']:substr($content['publish_up'],0,10); //$content['date'];
				$sysParams['#rasta-content-time#']			= (!empty($content['time']))?$content['time']:substr($content['publish_up'],10,8); //$content['time'];
			}
...
/////////////
	protected function _rtcTypeFn1($value)	// Normal Rtc
	{
		...
		if(isset($this->allPageContents['t'] ['data'] [$value['unic']] ['xal']))
		{
			if( !is_object($this->_XAL) ) $this->helper_ignite_XAL();
			$this->_XAL->setTheRunningMode('NORMAL_MODE');
			$this->page['wb_xml']['var:rtc']=$value;
			//print_r($this->page['wb_xml']); die();
			$this->page['wb_xml'] = $this->_XAL->run('<execution>'.$this->allPageContents['t'] ['data'] [$value['unic']] ['xal'].'</execution>', $this->page['wb_xml']);
			
			//print_r($this->page['wb_xml']); die("111");
			$value = $this->page['wb_xml']['var:rtc'];
			unset($this->page['wb_xml']['var:rtc']);
		}
	...
	public function helper_ignite_XAL($handler='')	
	{
		...
		else	$this->_XAL	= new Xal_Servlet('NORMAL_MODE');
	}	
